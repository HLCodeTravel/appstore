...
public class MoreDetailActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private FloatingActionButton mFloatActionButtonPlus;
...
//    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMoreUrl = getIntent().getStringExtra("moreurl");
        mResourceSnapshotId = getIntent().getStringExtra("moreSnapshotId");
    ...
    private void initData() {
        mRequestUrlName = ConstantUtils.generateImageName(mMoreUrl);
        mPath = AppStoreConstant.APPCENTER_JSON_DIR + mRequestUrlName + "_" + mResourceSnapshotId;
        if (ConstantUtils.checkNetworkState(BaseApplication.getContext())) {
            if (new File(mPath).exists()) {
                loadLocalData();
            } else {
                NetworkClient.startRequestQueryPageDetail(BaseApplication.getContext(), mMoreUrl, requestDataCallBack);
            }
        } else {
            if (new File(mPath).exists()) {
                loadLocalData();
    ...
    private void initView() {
        ...
        mFloatActionButtonPlus = (FloatingActionButton) findViewById(R.id.FabPlus);
        Glide.with(this).load(R.drawable.loading).asGif().diskCacheStrategy(DiskCacheStrategy.SOURCE).into(mLoadingProgress);
        mRecylerView = (RecyclerView) findViewById(R.id.rv_more);
        mTitle = (TextView) findViewById(R.id.category_top_bar_title);
        ...
        if (getIntent().getExtras().containsKey("moretitle")) {
            mTitle.setText(getIntent().getExtras().getString("moretitle"));
        }
        ...
        mFloatActionButtonPlus.setOnClickListener(this);
        mRecylerView.addOnScrollListener(new OnVerticalScrollListener() {
            @Override
            public void onScrolledUp() {
                ...
                if (mFloatBitmap != null && ConstantUtils.isShowFloatWindows(mBannerMapBean.getStartTime(), mBannerMapBean.getEndTime())) {
                    mFloatActionButtonPlus.setImageBitmap(mFloatBitmap);
                    mFloatActionButtonPlus.show();
                } else {
                    mFloatActionButtonPlus.hide();
                }
            }
    ...
    public void refreshView() {
        ...
        List<FloorMapBean> webstoreFloorMapList = mBaseBeanResult.getResultMap().getWebstoreFloorMapList();
        ...
        if (!TextUtils.isEmpty(imageUrl)) {
            Glide.with(BaseApplication.getContext()).load(imageUrl).asBitmap().centerCrop().into(new SimpleTarget<Bitmap>() {
                ...
                public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                    ...
                    mFloatActionButtonPlus.show();
                    mFloatActionButtonPlus.setImageBitmap(resource);
                }
            });
        } else {
            mFloatActionButtonPlus.hide();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.category_top_bar_back:
                ...
                param.putString("Action", "icon_back");
                FirebaseStat.logEvent("MoreDetailBack", param);
                break;
            case R.id.FabPlus:
                if (mBannerMapBean != null) {
                    ConstantUtils.onItemClickBannerCombiner(mBannerMapBean, MoreDetailActivity.this);
    ...
    protected void onResume() {
        ...
        FirebaseStat.setCurrentScreen(this, "MoreDetailActivity", null);
    }
